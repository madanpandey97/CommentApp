...
class RegistrationAPIView(generics.CreateAPIView):
    serializer_class = user_serializers.KAJABIUserCreateSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"status": True, "AccessToken": tokens["access"], "RefreshToken": tokens["refresh"]},
...
class ObtainTokenLogin(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "username" not in request.data:
            ...
            return Response(
                {"status": False, "username": "This field is required"}, status=status.HTTP_200_OK
            )
        if "password" not in request.data:
            logger.info(f"password not supplied required")
            return Response(
                {"status": False, "password": "This field is required"}, status=status.HTTP_200_OK
            )
        ...
        if not user:
            ...
            return Response({"status": False, "user": "This field is required"}, status=status.HTTP_200_OK)
        ...
        if user:
            ...
            return Response(
                {"message": "You have successfully Logged In", "status": True, "AccessToken": tokens["access"],
            ...
            return Response(
                {"message": "email or password does not match, please enter correct details", "status": False},
                status=status.HTTP_200_OK,
            )
