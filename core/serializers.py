...
class KAJABIUserCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if not attrs.get("password") == attrs.pop("confirm_password"):
            raise serializers.ValidationError(
                errors.UR_PASS_AND_CONFRM_PASS_DONT_MATCH
            )
        ...
        if attrs.get("phone"):
            if User.objects.filter(phone=attrs.get("phone")).exists():
                raise serializers.ValidationError(
                    errors.PHONE_NUM_ALREADY_REGISTERED
                )
        else:
...
class ChangePasswordSerializer(serializers.ModelSerializer):
    ...
    password = serializers.CharField(write_only=True)
    confirm_password = serializers.CharField(write_only=True)
    old_password = serializers.CharField(write_only=True, allow_null=True, allow_blank=True)
